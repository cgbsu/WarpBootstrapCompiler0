...
struct StrongFactor : public ConstexprStringable
{
    ...
    constexpr virtual std::string_view to_string() const override final
    {
        ...
        const char test[] = "StrongFactor memes are cool";
...
struct Node// : public ConstexprStringable//, public Owner
{
...
struct NodePointer//r : public Owner
{
    using PointerType = const Node*;
    const PointerType pointer;
    constexpr NodePointer( const PointerType other_pointer ) : pointer( other_pointer ) {}
    constexpr NodePointer( const NodePointer& other ) = default;// : pointer( other.pointer ) {}
    constexpr NodePointer( NodePointer&& other ) = default;// : pointer( other.pointer ) {}

    constexpr NodePointer& operator=( const NodePointer& other ) = default;
    constexpr NodePointer& operator=( NodePointer&& other ) = default;

    template< typename NodeType >
    constexpr static const NodePointer make_pointer( auto... constructor_values ) {
        const NodePointer new_pointer( new NodeType{ constructor_values... } );
        return new_pointer;
    }
...
// virtual 
    constexpr bool clean_up()// const override final
    {
...
struct SizeTStringable : public ConstexprStringable {
    const size_t value;
    constexpr SizeTStringable( size_t value ) : value( value ) {}
    constexpr virtual std::string_view to_string() const override final {
        return Utility::to_string< size_t >( value );
    }

    constexpr SizeTStringable operator*( const size_t other ) {
        return SizeTStringable{ value * other };
    }
    constexpr SizeTStringable operator/( const size_t other ) {
        return SizeTStringable{ value / other };
    }
    ...
    constexpr std::strong_ordering operator<=>( const auto other ) {
        return value <=> other;
    }
};
// using LiteralType = StrongFactor< size_t, long long int >;

using LiteralType = SizeTStringable;
...
constexpr static ctpg::nterm< const NodePointer > factor{ "Factor" };
constexpr static ctpg::nterm< const NodePointer > sum{ "Sum" };
constexpr static ctpg::nterm< const NodePointer > parenthesis_scope{ "ParenthesisScope" };
