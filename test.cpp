...
int main( int argc, char** args )
{
	...
	auto& call_parse = ConstantExpression< size_t >::parser;
	std::string input = "3 + 4";
	if( auto call_parse = call_parser.parse( ctpg::buffers::string_buffer( input.c_str() ), std::cerr );  
                  call_parse.has_value() == true )                                                                                                                                    
		std::cout << "Result: " << call_parse.value() << "\n";
	return 0;
}
